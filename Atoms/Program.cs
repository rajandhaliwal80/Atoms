...
using System.Linq;
using System.Text;
//add reference (right click project then add > refernce>framework>system.web.expression)
...
namespace ConsoleApplication1
{
    class Program
    {
        static List<Atom> elements;
        static void Main(string[] args)
        {
            Atom atom = new Atom() { Name = "Hydrogen", Symbol = "H", Proton = 1, Neutron = 0, Weight = 1.0079 };


            DisplayAtoms();

            SerializeOneAtomUsingXML(atom);

            Console.WriteLine("\nDeserializing ONE Atom(XML)\n");


            DeSerializeOneAtomUsingXml();

            SerializeAllAtomsUsingXml();

            Console.WriteLine("\nDeserializing ALL Atoms(XML)\n");

            DeSerializeAllAtomsUsingXml();

            SerializeOneAtomUsingJSON(atom, "Atom.json");

            Console.WriteLine("\nDeserializing ONE Atom(JSON)\n");

            DeserializeOneAtomUsingJSON("Atom.json");

            SerializeAllAtomsUsingJSON(elements, "Atoms.json");

            Console.WriteLine("\nDeserializing ALL Atoms(JSON)\n");


            DeserializeAllAtomsUsingJSON("Atoms.json");
        ...
        static void DeserializeOneAtomUsingJSON(string filename)
        {
            using (TextReader reader = new StreamReader(filename))
            {
                ...
                Console.WriteLine(
                    serializer.Deserialize<Atom>(reader.ReadToEnd()));
            }
        }
        static void DeserializeAllAtomsUsingJSON(string filename)
        {
            using (TextReader reader = new StreamReader(filename))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();

                List<Atom> allAtoms = serializer.Deserialize<List<Atom>>(reader.ReadToEnd());
                foreach (var item in allAtoms)
                {
                    Console.WriteLine(item);
                }
            }
        }
        static void SerializeAllAtomsUsingJSON(List<Atom> alist, string filename)
        {
            using (TextWriter writer = new StreamWriter(filename))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
                writer.WriteLine(serializer.Serialize(alist));
            }
        }
        static void SerializeOneAtomUsingJSON(Atom atom, string filename)
        {
            using (TextWriter writer = new StreamWriter(filename))
            {
                JavaScriptSerializer serializer = new JavaScriptSerializer();
        ...
        static void SerializeAllAtomsUsingXml()
        {
            //STEP II - create and initialize the serializer object
            XmlSerializer serializer = new XmlSerializer(typeof(List<Atom>));
            //STEP III - serialise to a stream
            TextWriter writer = new StreamWriter("Atoms.xml");
            serializer.Serialize(writer, elements);
            writer.Close();
        }
        static void DeSerializeAllAtomsUsingXml()
        {
            //STEP II - create and initialize the serializer object
            XmlSerializer serializer = new XmlSerializer(typeof(List<Atom>));
            //STEP III - deserialise from a stream
            TextReader reader = new StreamReader("Atoms.xml");
            List<Atom> atms = (List<Atom>)serializer.Deserialize(reader);
        ...
        static void DeSerializeOneAtomUsingXml()
        {
            //STEP II - create and initilize the serializer object
            XmlSerializer serializer = new XmlSerializer(typeof(Atom));
            //STEP III - deserialize from a stream
            TextReader reader = new StreamReader("Atom.xml");
            Atom s = (Atom)serializer.Deserialize(reader);
            Console.WriteLine(s);
            reader.Close();
        }
        public static void SerializeOneAtomUsingXML(Atom atom)
        {
            //STEP II - create and initilize the serializer object
            XmlSerializer serializer = new XmlSerializer(typeof(Atom));
            //STEP III - serialize to a stream
            TextWriter writer = new StreamWriter("Atom.xml");
            serializer.Serialize(writer, atom);
            writer.Close();

        }

        public static void DisplayAtoms()
        {
            elements = new List<Atom>();


            //Create a static method and insert the following code
            ...
            elements.Add(Atom.Parse("Protactinium 91 140 231.0359"));
